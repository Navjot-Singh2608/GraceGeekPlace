...
angular.module('myProfile').controller('profileCtrl', ['$scope', '$rootScope', '$state', '$http', 'toastr', 'profileService','$window', function($scope, $rootScope, $state, $http, toastr, profileService,$window) {
   ...
   /******************************************hire Profile functionality**************************************/
    ...
    $scope.getAllFilesNames = function(){
        profileService.getAllFilesNames().then(function (response) {
           console.log(response.data);
            angular.forEach(response.data, function(userObj,userKey) {
                var n = userObj.lastIndexOf('/');
                var fileName = userObj.substring(n + 1);
                fileName = fileName.replace(/\\/g, '');
                $scope.filesNames.push(fileName)
            })
        })
    };

    $scope.getAllFilesNames();
