...
var executeQuery = function executeQuery(makeRequest) {
  return function (query) {
    ...
    return _Rx.Observable.fromPromise(request);
  };
};
...
var toPages = function toPages(response, row$, options) {
  ...
  return row$.filter(notYetLoaded).map(function (rowNum) {
...
var DataSource = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(DataSource, [{
    ...
    value: function createQueryStream(initialQuery) {
      ...
      var pagedData$ = initialData$.flatMap(function (wrappedInitialResponse) {
        var initialResponse = wrappedInitialResponse.response;
        var row$ = toPages(initialResponse, rowRequest$, _this2.options).map(function (paging) {
          return _objectSpread({}, initialResponse.meta.query, {
            paging: paging
          });
        }).flatMap(executeQuery(_this2.makePagedRequest)).scan(function (mergedResponse, wrappedPageResponse) {
    ...
    value: function createRowStream() {
      var stream = new _Rx.ReplaySubject();
